...
public class PDFController {
	...
	private static float EINZUG_OBEN = 138;					// 3 * Zellenbreite Tabellenheader + Einzug Oben = 138.
	private static float EINZUG_UNTEN = 36;
	private static int HEADER_STARTZEILE = 0;
	private static int HEADER_ENDZEILE = -1;
	private static float HEADER_X_POS = 36;
	private static float HEADER_Y_POS = 809;				// 845 wäre Tabelle ganz oben. 845 - 36 Einzug gibt 809
	private static float HEADER_TABELLENBREITE = 523;		// 595 wäre Tabelle über ganze A4-Breite 595 - 2*36 Einzug gibt 523
	private static float HEADER_TABELLENPADDING = 2;
	private static float HEADER_TABELLENSCHRIFTGROESSE = 30;
	private static Font FONT_DOKUMENTHEADER = FontFactory.getFont(FontFactory.HELVETICA, HEADER_TABELLENSCHRIFTGROESSE, Font.BOLDITALIC, BaseColor.BLACK);
	...
	private static Font FONT_TABELLENHEADER = FontFactory.getFont(FontFactory.HELVETICA, 16, Font.BOLDITALIC, BaseColor.BLACK);
	
	
	/**
	 * Die innere Klasse "Dokumentheader" erweitert die Klasse "PdfPageEventHelper", welche von der iText-Library bereitgestellt wird.
	 * Damit ein Dokumentheader erzeugt werden kann, der auf jeder Seite erscheint. Muss diese Klasse existieren.
	 * 
	 * @author Aebischer Patrik, Bösiger Elia, Gestach Lukas
	 * @date 09.05.2017
	 * @version 1.0
	...
	static class Dokumentheader extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document dokument) {
			header.writeSelectedRows(HEADER_STARTZEILE, HEADER_ENDZEILE, HEADER_X_POS, HEADER_Y_POS, writer.getDirectContent());
		}
	}
	/*
	// TODO: Nur zum testen.
	public static void main(String args[]) throws IOException, DocumentException {
		...
		daten.add(rangliste);
		daten.add(rangliste);
		new PDFController().generierePDF(dest, rennen, "Startliste", titel, HEADER, daten);
	}*/

	/**
	 * Generiert eine oder mehrere Tabellen in einem PDF-Dokument.
	 * @param pfad - Speicherort des Dokuments
	 * @param rennen - Aktuelles Rennen
	 * @param dokumenttitel - Titel des Dokuments
	 ...
	 */
	public static void generierePDF(String pfad, Rennen rennen, String dokumenttitel, List<String> tabellentitel, String[] header, List<List<List<String>>> daten)
		...
		Document dokument = new Document(PageSize.A4, EINZUG_LINKS, EINZUG_RECHTS, EINZUG_OBEN, EINZUG_UNTEN);
		...
		for (int i = 0; i < anzTabellen; i++) {
			...
			for (int j = 0; j < anzSpalten; j++) {
				...
				zelle.setBackgroundColor(BaseColor.GRAY);
