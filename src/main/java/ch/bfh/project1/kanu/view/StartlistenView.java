...
public class StartlistenView implements ViewTemplate {
	...
	// private Button gesamteStartlisteAlsPDF = new Button("Gesamte Startliste
	...
	public void viewAnzeigen(Component inhalt) {
		...
		if (rennen == null) {
			lrennen = sController.ladeAlleRennen(); // Wenn kein Rennen
													// ausgewählt, eine Liste
													// mit Rennen ausgeben
			zeigeRennen(inhalt);
			...
			kategorien.clear();
			for (AltersKategorie kat : rennen.getKategorien()) // Kategorien
																// ausgeben
			{
			...
			for (Integer k : kats) {
				kategorien.removeItem(k); // Bereits gesetzte Kategorien wieder
											// entfernen
			}
			...
			for (ListSelect ls : bloecke) // Blöcke aus der Datenbank anzeigen
			{
				...
				if (anzBloecke == 1) // Beim ersten Block nebenan noch die
										// verbleibenden Kategorien anzeigen
				{
			...
			if (bloecke.size() == 0) // Wenn keine Blöcke in der db gefunden
										// wurden, einen neuen anzeigen
			{
	...
	/**
	 * Zeigt die verschiedenen Rennen an, zu denen dann die Blöcke und
	 * Startlisten konfiguriert werden können
	 * 
	 * @param inhalt
	 *            Das Inhalts Panel
	 */
	...
	private void zeigeStartliste() {
		vLayout.removeAllComponents();
		Label startlistenLabel = new Label("Startliste");
		startlistenLabel.setStyleName("h2");
		vLayout.addComponent(startlistenLabel);
			...
			if (altKat != f.getKategorie().getAltersKategorieID()) {
				...
				if (res.size() > 0) {
					...
					for (FahrerResultat r : res) {
						...
						row.getItemProperty(Startliste.NAME)
